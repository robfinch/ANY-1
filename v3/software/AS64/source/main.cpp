...
void displayHelp()
{
     ...
     printf("                  I=Itanium  D=DSD6 7=DSD7 A=DSD9 F=FT64 G=FT64x36 X=FT8088 N=NVIO3\r\n");
		 printf("                  O=Gambit  O5=Gambit_v5 R=RTF64 Y=ANY1 y=ANY1V3\r\n");
  ...
  */
  if (segment == codeseg || segment == rodataseg) {
    ...
    case codeseg:
      ...
      if (gCpu == ANY1V3) {
        code_address++;
        if ((code_address & 63LL) >= 60LL) {
          code_address &= ~63LL;
          code_address += 64LL;
        }
        if ((sections[segment].address & 63LL) >= 60LL) {
          sections[segment].address &= ~63LL;
          sections[segment].address += 64LL;
        }
        if ((sections[segment].index & 63LL) >= 60LL) {
          sections[segment].index &= ~63LL;
          sections[segment].index += 64LL;
        }
      }
      else
      */
...
void processMaster()
{
  ...
  case ANY1:  any1_processMaster(); break;
  case ANY1V3:  any1v3_processMaster(); break;
    ...
    if (verilog_out) {
        ...
        if (vfp) {
			...
			*/
			...
			else if (gCpu == 'F' || gCpu == 'G' || gCpu == 'J' || gCpu==RTF64 || gCpu==ANY1 || gCpu==ANY1V3) {
				if (vebits == 256) {
