...
#define RB(x)		(((x) & 0x3fLL) << 20LL)
#define FUNC4(x)	(((x) & 15LL) << 28LL)
#define FUNC6(x)	(((x) & 0x3FLL) << 26LL)
#define IMM(x)	(((x) & 0xfffLL) << 20LL)
...
#define BRDISP(x)	((((x) >> 4LL) & 0x3fLL) << 26LL) | (((x)&15LL)<<10LL)
...
static void LoadConstant(int64_t val, int rg)
{
	...
	if (IsNBit(val, 32LL)) {
		...
		emit_insn(((val & 0xFFFLL) << 20LL) | RT(rg) | I_ADDI);
		return;
	}
	if (IsNBit(val, 56LL)) {
		emit_insn(((val >> 8LL) << 8LL) | I_EXI0);
		emit_insn(((val >> 32LL) << 8LL) | I_EXI1);
		emit_insn(((val & 0xFFFLL) << 20LL) | RT(rg) | I_ADDI);
		return;
	}
	emit_insn(((val >> 8LL) << 8LL) | I_EXI0);
	emit_insn(((val >> 32LL) << 8LL) | I_EXI1);
	emit_insn(((val >> 56LL) << 8LL) | I_EXI2);
	emit_insn(((val & 0xFFFLL) << 20LL) | RT(rg) | I_ADDI);
	return;
}
...
static void process_bcc()
{
	...
	if (!IsNBit(disp,11))
		emit_insn((((disp) >> 8LL) << 20LL) | RA(63) | RT(0) | I_BRMOD);
	emit_insn((((disp) >> 6LL) << 26LL) | RB(Rb) | RA(Ra) | (((disp >> 2LL)  & 15LL) << 10LL) | opcode6);
	prevToken();
	return;
}
...
static void process_call(int opcode, int opt)
{
	...
	if (Ra == 0 && !rel) {
		if (code_bits < 25) {
			emit_insn(((val >> 2LL) << 10LL) | ((lk) << 8LL) | I_JAL);
			return;
		}
		if (IsNBit(val, 24)) {
			emit_insn(((val >> 2LL) << 10LL) | ((lk) << 8LL) | I_JAL);
	...
	if (Ra == 0 && rel) {
		val = disp;
		if (code_bits < 25) {
			emit_insn(((val >> 2LL) << 10LL) | ((lk) << 8LL) | I_BAL);
			return;
		}
		if (IsNBit(val, 24)) {
			emit_insn(((val >> 2LL) << 10LL) | ((lk) << 8LL) | I_BAL);
...
static void process_store()
{
  ...
  mem_operand(&disp, &Ra, &Rc, &Sc, &seg);
	if (Ra >= 0 && Rc >= 0) {
		emit_insn((Sc>1 ? (1LL << 20LL) : 0LL) | RA(Rc)|I_IMOD);
		emit_insn(
			FUNC4(opcode6) |
			RT(0) |
			RA(Ra) |
  ...
  val = disp;
	...
	if (IsNBit(val, 8))
		emit_insn(FUNC4(opcode6) | ((val >> 6LL) << 26LL) | RB(Rs) | RA(Ra) | RT(val) | I_STx);
	else if (IsNBit(val, 32)) {
		emit_insn((((val) >> 8LL) << 8LL) | I_EXI0);
		emit_insn(FUNC4(opcode6) | ((val >> 6LL) << 26LL) | RB(Rs) | RA(Ra) | RT(val) | I_STx);
	}
	else if (IsNBit(val, 56)) {
		emit_insn((((val) >> 8LL) << 8LL) | I_EXI0);
		emit_insn((((val) >> 32LL) << 8LL) | I_EXI1);
		emit_insn(FUNC4(opcode6) | ((val >> 6LL) << 26LL) | RB(Rs) | RA(Ra) | RT(val) | I_STx);
	}
...
static void process_load()
{
  ...
  if (Ra < 0) Ra = 0;
	...
	if (Ra >= 0 && Rb >= 0) {
		emit_insn(FUNC4(opcode6)| (Sc > 1 ? (1LL << 26LL) : 0LL) | RB(Rb)|RA(Ra)|RT(Rt)|(opcode6&0x80?I_LDxXZ:I_LDxX));
		ScanToEOL();
		return;
	}

	if (IsNBit(val, 8)) {
		emit_insn(FUNC4(opcode6) | ((val & 0xffLL) << 20LL) | RA(Ra) | RT(Rt) | (opcode6 & 0x80 ? I_LDxZ : I_LDx));
	}
	else if (IsNBit(val, 32)) {
		emit_insn((((val) >> 8LL) << 8LL) | I_EXI0);
		emit_insn(FUNC4(opcode6) | ((val & 0xffLL) << 20LL) | RA(Ra) | RT(Rt) | (opcode6 & 0x80 ? I_LDxZ : I_LDx));
...
static void process_cache(int opcode6)
{
  ...
  mem_operand(&disp, &Ra, &Rb, &Sc, &seg);
	...
	if (IsNBit(val, 8)) {
		emit_insn(FUNC4(opcode6) | ((val & 0xffLL) << 20LL) | RA(Ra) | RT(cmd) | ((opcode6 & 0x80) ? I_LDxZ : I_LDx));
	}
	else if (IsNBit(val, 32)) {
		emit_insn((((val) >> 8LL) << 8LL) | I_EXI0);
		emit_insn(FUNC4(opcode6) | ((val & 0xffLL) << 20LL) | RA(Ra) | RT(cmd) | ((opcode6 & 0x80) ? I_LDxZ : I_LDx));
